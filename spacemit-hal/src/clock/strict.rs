...
impl Strict {
    ...
    pub fn configure<'a>(
        ...
        let pll1 = self.validate_pll(
            apbs.pll1_software_control1.read(),
            apbs.pll1_software_control3.read(),
            mpmu.pll_status.read().is_pll1_locked(),
        )?;
        let counter = counter.frequency(self.reference.ok_or(Error::UnsupportedPll)?)?;
        let (parent, gates) = self.gates(
            apbs.pll1_software_control2.read(),
            mpmu.application_clock_gate.read(),
        );
        if self.i2c.is_some() || self.uart.is_some() {
            // SAFETY: Instance grants exclusive configuration access; live HAL
            // consumers retain these borrows and exclude reconfiguration.
            unsafe {
    ...
    fn validate_pll(self, control1: u32, control3: u32, locked: bool) -> Result<Hertz, Error> {
        ...
        if self.reference != Some(Hertz(24_000_000))
...
fn uart_sources(
    ...
    #[inline]
    fn slow(parent: u32, value: u32) -> Option<Hertz> {
    ...
    let d4 = parent & (1 << 3) != 0;
    [
        (d8 && gates & (1 << 8) != 0).then_some(Hertz(pll1.0 / 128 * 3)),
...
mod tests {
    ...
    fn uart_binding_checks_its_static_register_without_writing() {
        ...
        clocks.uart[0] = Some(Hertz(57_600_000));
    ...
    fn uart_frequencies_follow_live_paths() {
        ...
        let slow2 = (32 << 16) | 5;
        assert_eq!(
            uart_sources(pll, parent, gates, slow1, slow2),
            [
                Some(Hertz(57_600_000)),
                Some(Hertz(14_745_600)),
