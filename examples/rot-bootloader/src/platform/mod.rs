...
#[doc(hidden)]
pub use init_board_k1_m1 as init_board;
...
impl Board {
    ...
    ///
    /// DRAM must contain no live data; retain link.x's firmware, stack and guard regions.
    ///
...
/// Initializes the shared K1/M1 board devices.
pub fn init_board_k1_m1(p: &'static mut Peripherals) -> Result<Board> {
...
// Keep each device's temporary setup state out of the shared initialization frame.
#[inline(never)]
fn init_console(
