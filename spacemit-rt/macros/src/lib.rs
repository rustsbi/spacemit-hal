...
use quote::quote;
use syn::{ItemFn, ReturnType, Type, Visibility, parse_macro_input, spanned::Spanned};

/// Declares a parameterless entry function for every hart.
...
fn validate(args: Tokens, f: &ItemFn) -> syn::Result<()> {
        ...
        || !output
    {
        return Err(syn::Error::new(
            f.span(),
            "expected a private parameterless function with return type () or !",
        ));
    }
    for attr in &f.attrs {
...
fn expand(f: ItemFn, runtime: Tokens) -> Tokens {
    ...
    quote!(
        // The image linker and loader must satisfy spacemit-rt's startup contract.
        #[unsafe(export_name = "_start")]
        #[unsafe(link_section = ".text.entry")]
        #[unsafe(naked)]
        unsafe extern "C" fn _start() -> ! {
            core::arch::naked_asm!("tail {}", sym #runtime::start);
        }
...
mod tests {
    ...
    fn preserves_function_name_with_fixed_export() {
        ...
        let syn::Item::Fn(entry) = &expanded.items[1] else {
